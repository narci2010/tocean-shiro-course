package com.toceansoft.shiro.demo8.session.scheduler;
...
import com.toceansoft.shiro.demo8.JdbcTemplateUtils;
import com.toceansoft.shiro.demo8.SerializableUtils;
...
public class MySessionValidationScheduler implements SessionValidationScheduler, Runnable {
	...
	public void run() {
			...
			for (String sessionStr : sessionList) {
				try {
					...
					Method validateMethod = ReflectionUtils.findMethod(
							AbstractValidatingSessionManager.class, "validate", Session.class,
							SessionKey.class);
		...
		if (log.isDebugEnabled()) {
			log.debug("Session validation completed successfully in " + (stopTime - startTime)
					+ " milliseconds.");
		}
	}
