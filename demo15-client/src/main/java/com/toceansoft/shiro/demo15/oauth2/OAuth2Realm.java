...
public class OAuth2Realm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token)
		...
		// 没有设置CredentialsMatch，无需进一步密码验证
		SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(username, code,
				getName());
	...
	private String extractUsername(String code) {
		...
		try {
			...
			OAuthClientRequest accessTokenRequest = OAuthClientRequest.tokenLocation(accessTokenUrl)
					...
					.setClientSecret(clientSecret).setCode(code).setRedirectURI(redirectUrl)
					.buildQueryMessage();
